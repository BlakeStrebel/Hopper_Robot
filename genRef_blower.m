function ref = genRef_blower(reflist)
...
%
%   Output:
%       An array ref, each element representing the reference frequency, in Hz,
%       spaced at time 1/f, where f is the frequency of the trajectory controller.
%       Also plots ref.
%       
%   Example usage:  ref = genRef([0, 0; 1.0, 30; 1.5, 30; 2.5, 0]);
%
%   The via points are 0 Hz at time 0 s; 30 Hz at time 1 s; 
%   30 Hz at 1.5 s; and 0 Hz at 2.5 s.
% 
%   Note:  the first time must be 0, and the first and last velocities should be 0.

MOTOR_SERVO_RATE = 10;      % 20 Hz control frequency
...
% calculate a linear interpolation trajectory  
  ...
  if numpos >= 3
    for i=1:numpos-1
      vellist(i) = (poslist(i+1)-poslist(i))/(timelist(i+1)-timelist(i));
    end
  end
  ...
  for i=1:numpos-1            % go through each segment of trajectory
    ...
    a0 = posstart;            % calculate coeffs of traj pos = a0+a1*t
    a1 = velstart;
    while (refCtr-1)*dt < timelist(i+1)
      tseg = (refCtr-1)*dt - timelist(i);
      ref(refCtr) = a0 + a1*tseg;  % add an element to ref array
      refCtr = refCtr + 1;
    end
  end
