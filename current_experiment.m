...
mode = 'linear';
filename = sprintf('c_%s_%.3f_%.3f.txt',mode,max(trajectory(:,1)),max(trajectory(:,2)));
...
selection = 'u';    % load trajectory
plunge = 'v';       % execute trajectory
...
stepsx = 3;     % number of steps in x direction
stepsy = 7;     % number of steps in y direction

for i = 1:stepsx
   for j = 1:stepsy
        fprintf(NU32_Serial,'%c\n',plunge);             % tell PIC32 to plunge
        data = read_plot_matrix_current(NU32_Serial,0); % read data back from PIC32
        plot(times,data(:,2));                          % plot data
        
        fprintf(fileID,'\r\ntrial%d\r\n',i+j); 
        for ii = 1:size(data,1)         
            fprintf(fileID,'%f %f\r\n',data(ii,1),data(ii,2));  % write data to text file
        end
