function ref = genRef_current(reflist, method)
...
%
%   Output:
%       An array ref, each element representing the reference position, in A,
...
% convert the list of times to a list of sample numbers
  ...
  for i=2:numpos
    ...
    for samp = last_sample:(sample_list(i)-1)
      ref(samp+1) = pos_list(i-1);
    end
    last_sample = sample_list(i)-1;
  end
  ref(samp+1) = pos_list(end);
