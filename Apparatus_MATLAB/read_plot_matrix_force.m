...
data = zeros(nsamples,6);               % two values per sample:  ref and actual

for i=1:nsamples
    data(i,1:5) = fscanf(mySerial,'%d %f %d %d %d'); % read in data from PIC32; position (um); current (amps); force (counts)
end
...
if nsamples > 1						        
    ...
    stairs(times,data(:,[1, 2, 3, 6]));            % plot the reference and actual
    % compute the average error
    score = mean(abs(data(:,6)-data(:,3)));
