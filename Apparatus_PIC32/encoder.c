...
static int encoder_command(int read) { // send a command to the encoder chip
  ...
  SPI3BUF = 9;                         // write garbage, but the read will have the data
...
void encoder_init(void) {
  ...
  TRISDbits.TRISD0 = 0;	// setup chip select pin as output
  CS = 1;
  ...
  SPI3BRG = 3999;            // bit rate to 20 MHz, SPI4BRG = 80000000/(2*desired)-1
